...
exports.WebchatPlugin = class WebchatPlugin extends BaseAction {
  ...
  elements = {
    ...
    webchatTriggerMessage: 'li[data-from="SYSTEM"]:last-child div div:nth-child(2)',
  };
  ...
  async verifyLayout(msgIndex){
    ...
    /**
   * function to validate the message
   * @param {string} triggerMessage - triggered configure message
   * @return {void} Nothing
   */
    async validateTriggerMessage(triggerMessage){
      await this.switchTab('newTab');
      // waiting to load text on DOM
      await this.wait(5);
      await this.containSubstring(
        await frames.locator(this.elements.webchatTriggerMessage).innerText(),
          triggerMessage
        ); 
    }
